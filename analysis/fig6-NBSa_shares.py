...
for zone, ax in zip(zones, axes):
    ...
    if firstCol:
        ax.set_ylabel("Share of NBS-assets")
        ax.yaxis.set_label_coords(x=-.1, y=0.45)
...
ax.text(-(nb_exps+1.1), pM_xlabel_height-0.4, "Discovered by:", ha="right", va="bottom")

# Add legend using the labels and handles from the last subplot
fig.legend(handles=legend_elements, loc=(0.165, 0.775))
fig.tight_layout()
#filename=f"{FIGS_DIR}/{','.join(EXP_LIST)}-{str_specs}-acqshare-cons-behav-prov-behav-werr_{step}-{BEGIN}-{END}-{str_vals}.pdf"
filename=f"{FIGS_DIR}/{','.join(EXP_LIST)}-acqshare-cons-behav-prov-behav-werr_{step}-{BEGIN}-{END}-{str_vals}.pdf"
