...
run_batches() 
{
    ...
    do
        for ((j=0;j<BATCH;j++));
        do
            (echo "process $i started" && OMNETPP_RELEASE=$OMNETPP_RELEASE OMNETPP_IMAGE_PATH=$OMNETPP_IMAGE_PATH ../src/$EXE_NAME -r $i -m -u Cmdenv -c $5 -n ".;../src" omnetpp.ini --record-eventlog=false --scalar-recording=false --vector-recording=false > $LOG_DIR$EXPERIMENT_NAME-$i.log && echo "process $i finished") &
            i=$(($i+1))
            if test $i -ge $END
            then
                break
...
OMNETPP_RELEASE=$OMNETPP_RELEASE OMNETPP_IMAGE_PATH=$OMNETPP_IMAGE_PATH ../src/$EXE_NAME -c $5 -n ".;../src" omnetpp.ini -q runs > $LOG_DIR$4-runs.log
